...
The Forgejo exact-head suite validates the candidate's local HACS source
schemas and hassfest results without exposing a GitHub token. It separately
validates stable public GitHub repository properties through the anonymous API.
After an exact commit is mirrored, the downstream GitHub workflow may add full
HACS repository validation. A missing or failed affected-component gate at
either boundary blocks release.
